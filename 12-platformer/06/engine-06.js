...
/* There is a minor update since Part 01 due to a minor mistake. Instead of calling
window.requestAnimationFrame before updating the game logic, it was being called after.
This meant that even if we tried to stop the game loop from inside the game logic,
requestAnimationFrame would still be called after we requested a stop. To fix this,
simply move the request to the top of my game loop function, which is Engine.run. */
...
const Engine = function(timeStep, update, render) {
  ...
  this.run = function(timeStamp) {
    /* This line was moved from the bottom of this function to the top. This is better
    anyway, because it ensures that if the game logic runs too long, a new frame will
    already be requested before 30 or 60 frames pass and we miss a request entirely.
    This could cause a "spiral of death" for the CPU, but since we have the frame dropping
