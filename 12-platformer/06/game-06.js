...
This Game class contains the following changes since 05:
1. Simplified the Class structure to remove multiple levels, e.g. Game.World.Object.Player
   is now just Game.Player. Also put them in rough alphabetical order within Game
2. Added Game.World.prototype.setup() method to setup the world from the JSON zone data
3. Added the Game.MovingObject class to separate Objects from MovingObjects. Game.Player
   now inherits from MovingObject instead of Object
4. Changed the Game.Collider.collideObject() method to do all the y collision checks first.
   This simply means we check the collision on top and bottom before left and right
5. Removed the world boundary collision from World.collideObject so the player can now
   move off the screen enough to collide with a door. Also updated the World.update()
   method to check for door collisions
6. Added the Game.Door class to handle door functionality
7. Added methods to get the centre position of Game.Object and Game.MovingObject
8. Added a limit to the player velocity in order to avoid tunnelling.
9. Changed the players size and frame offsets to improve animation
10. Updated Game.Player.jump() method so the player can only jump if it isn't falling
    faster than 10px per frame.
*/
...
Game.Door.prototype = {
  ...
  // Method to test for a collision between the door and a MovingObject
...
Game.TileSet = function(columns, tileSize) {
  ...
  this.frames = [
    ...
    new frame(102,  96, 13, 16, 0, -4),  // walk-left images (+ next 3)
    ...
    new frame( 13, 112, 13, 16, 0, -4),  // walk-right image (+ next 3)
