...
Changes since 07:
1. Moved Carrot Count <p> element to HTML and styled it through CSS and controlled here
*/
...
window.addEventListener("load", function() {
  ...
  // Get the Carrot Count Element
  const carrotCountEl = document.getElementById("carrotCount");
  carrotCountEl.textContent = "Carrots: 0";
  ...
  const resize = function() {
    ...
    carrotCountEl.style.right = `${rectangle.left + 15}px`;
    carrotCountEl.style.top = `${rectangle.top}px`;
    carrotCountEl.style.fontSize = `${game.world.tileSet.tileSize * rectangle.height / game.world.height}px`;
  };
  ...
  const render = function() {
    ...
    carrotCountEl.textContent = `Carrots: ${game.world.carrotCount}`;
  ...
  const display = new Display(canvasEl);  // The display handles the canvas and window resizing
